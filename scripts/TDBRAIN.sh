                        ...
                        for patch_len in "${patch_lens[@]}"; do
                            for llm_layer in "${llm_layers[@]}"; do
                                python -u run.py \
                                    ...
                                    --dropout $dropout \
                                    --patch_len $patch_len\
                                    --augmentations none,frequency0.2,drop0.4 \
                                    --init zeros\
                                    --lradj constant \
                                    --itr 5 \
                                    --learning_rate $lr \
                                    --train_epochs 35 \
                                    --patience 35 > "${log_dir}/bs${bs}_lr${lr}_tl${t_layer}_cl${c_layer}_llm_${llm_layer}_dp${dropout}_dm${d_model}_pl${patch_len}.log"
                            done
                        done
                    done
                done
